...
namespace M8ScoreLibrary {
	...
	public class Match {
		...
		public Match():this(new MatchSettings()){
		...
		public void CalculateMatch() {
			...
			if(!unplayedSet) {
				...
				if(totalOpponentPlayerRatings > OverUnderLimit) {
					OpponentTeamBonusOrPenalty = -(totalOpponentPlayerRatings - OverUnderLimit) * PenaltyMultiplier;  
				} else {
					OpponentTeamBonusOrPenalty = theyForfeit ? 0 : (OverUnderLimit - totalOpponentPlayerRatings);
				}
		...
		public ScenarioResult DetermineMustGet() {
			...
			Match testMatch = null; //(Match)this.MemberwiseClone();
			using(var stream = new MemoryStream()) {
				...
				testMatch = (Match)binaryFormatter.Deserialize(stream);
			}
			...
			for(int i =0; i < 15; i++) {
				...
				Debug.WriteLine(string.Format("Opponent Score: {0}, must get to: {1}", testMatch.Sets[4].OpponentScore, mustGetScore));
			}
		...
		public ScenarioResult DetermineHoldThemTo() {
			...
			for(int i =0; i < 15; i++) {
				...
				Debug.WriteLine(string.Format("Score: {0}, must hold opponent to: {1}", testMatch.Sets[4].Score, maxOpponentScore));
			}
